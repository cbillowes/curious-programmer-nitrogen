import React from 'react'
import { mustContainValue, containsElement } from './_helpers'
import Footer from '../footer'
import Anchor from '../anchor'
import License from '../license'

const srcLinkedIn = "data:image/svg+xml;utf8;base64,"
const srcTwitter = "data:image/svg+xml;utf8;base64,"
const srcGitHub = "data:image/svg+xml;utf8;base64,"

function sut(props) {
  return (
    <Footer />
  )
}

const SocialIcon = (props) => {
  return (
    <Anchor
      to={props.to}
      title={props.title}
    >
      <img
        className="icon"
        src={props.src}
        alt={props.title}
      />
    </Anchor>
  )
}

const LinkedInSocialIcon = () => {
  return (
    <SocialIcon
      to="https://www.linkedin.com/in/cbouwer"
      title="LinkedIn"
      src={srcLinkedIn}
    />
  )
}

const TwitterSocialIcon = () => {
  return (
    <SocialIcon
      to="https://twitter.com/cbillowes"
      title="Twitter"
      src={srcTwitter}
    />
  )
}

const GitHubSocialIcon = () => {
  return (
    <SocialIcon
      to="https://github.com/cbillowes"
      title="GitHub"
      src={srcGitHub}
    />
  )
}

describe(`Footer`, () => {

  describe(`should be styled`, () => {

    it(`with class for footer`, () => {
      const match = mustContainValue(
        sut({}),
        `class="footer"`
      )
      expect(match).toBeTruthy()
    })

    it(`with class for container`, () => {
      const match = mustContainValue(
        sut({}),
        `class="container"`
      )
      expect(match).toBeTruthy()
    })

    it(`with class for social`, () => {
      const match = mustContainValue(
        sut({}),
        `class="social"`
      )
      expect(match).toBeTruthy()
    })

  })

  describe(`should display`, () => {

    it(`the slogon`, () => {
      const match = mustContainValue(
        sut({}),
        `A curious place for a curious mind.`
      )
      expect(match).toBeTruthy()
    })

    it(`the copyright for the given year`, () => {
      const match = mustContainValue(
        sut({}),
        `Copyright © ${new Date().getFullYear()}`
      )
      expect(match).toBeTruthy()
    })

  })

  describe(`should render`, () => {

    it(`the credits anchor`, () => {
      const match = containsElement(
        sut({}),
        <Anchor
          to="/credits"
        >
          cool stuff
        </Anchor>
      )
      expect(match).toBeTruthy()
    })

    it(`licenses`, () => {
      const match = containsElement(
        sut({}),
        <span className="licenses">
          <License />
        </span>
      )
      expect(match).toBeTruthy()
    })

    describe(`social media icons`, () => {

       it(`LinkedIn`, () => {
        const match = containsElement(
          sut({}),
          <LinkedInSocialIcon />
        )
        expect(match).toBeTruthy()
      })

      it(`Twitter`, () => {
        const match = containsElement(
          sut({}),
          <TwitterSocialIcon />
        )
        expect(match).toBeTruthy()
      })

      it(`GitHub`, () => {
        const match = containsElement(
          sut({}),
         <GitHubSocialIcon />
        )
        expect(match).toBeTruthy()
      })

    })

  })
})